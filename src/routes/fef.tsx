...
function ColumnFilter({
  ...
  column: { id: string; getFilterValue: () => unknown; setFilterValue: (v: unknown) => void };
  data: FefRow[];
}) {
  const value = (column.getFilterValue() ?? "") as string;
  const options = Array.from(new Set(data.map((row) => row[column.id as keyof FefRow]))).sort();
...
function FieldEstimateForm() {
  const [data, setData] = React.useState<FefRow[]>(initialData);
  ...
  return (
    <main className="p-4">
      <h1 className="text-2xl font-bold mb-4">Field Estimate Form</h1>
